...
public class MongoDatabaseWrapper implements Database {
	...
	/*
	 * (non-Javadoc)
	 * @see com.tap.project.escrivaghera.AccountantApp.Database#getAllRegistration(java.util.Date, java.util.Date)
	 * 
	 * For implement the getAllRegistraction between two date we take the list of all registrations
	 * (with the getAllRegistraction method without parameters) and then we delete from that list
	 * the intruders element, that is the records that have got the date out of range.
	 */
	@Override
	...
	/*
	 * For create the list of JounrnalEntry's object we need to create the JournalEntries's object
	 * from the database info. We note that the record format is:
	 * |   id|   date|  description|   value|  isLeft|
	 * First of all, we take the info for create a JounalEntry object using the class constructor
	 * In the second part, we take the info from the record for create the first Count and we add
	 * it to the list of Count object
	 * Finally, we create all other Count object analyzing the subsequent record that have got equal
	...
	private List<JournalEntry> getAllRegistration() {
		...
		while(i<myCursorList.size()){
			...
			try {
				//we create the first Count's object (the current record's Count object)
				...
				//we create the other Count object from the subsequent records info
				boolean exit=false;
